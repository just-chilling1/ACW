...
import { regenerateAssetContent } from "@/lib/instant/content-engine";
...
export async function POST(req: Request, { params }: RouteParams) {
    ...
    try {
        ...
        const option = clampString(body.option, 50) || "completely_different";
        const snapshot = kit.offer_snapshot as OfferSnapshot;

        const regenerated = await regenerateAssetContent(
            snapshot,
            kit.offer_url,
            asset.content,
            asset.type,
        ...
        const { data: updated, error } = await auth.supabase
            .from("promotion_assets")
            .update({ content: regenerated, updated_at: new Date().toISOString() })
            .eq("id", assetId)
            .select("*")
            .single();

        if (error) return NextResponse.json({ error: "Could not regenerate content." }, { status: 500 });
        return NextResponse.json({ asset: updated });
    } catch {
        return NextResponse.json({ error: "Could not regenerate content." }, { status: 500 });
    }
}
