...
import { getVaultKitWithAssets } from "@/lib/vault/kit-engine";
...
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const result = await getVaultKitWithAssets(auth.supabase, id, auth.user.id);
...
export async function PATCH(req: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (body.nicheId && isNicheId(clampString(body.nicheId, 40))) {
      updates.niche_id = clampString(body.nicheId, 40);
    }
