...
import { runCustomReplyGeneration } from "@/lib/dfy/reply-engine";
...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!problemSolved) {
            return NextResponse.json({ error: "Please describe the problem they need solved." }, { status: 400 });
        }
        if (!offerUrl) {
            return NextResponse.json({ error: "Offer URL is required." }, { status: 400 });
        }

        const result = await runCustomReplyGeneration(auth.supabase, auth.user.id, {
            niche,
            idealCustomer,
            problemSolved,
            offerUrl,
