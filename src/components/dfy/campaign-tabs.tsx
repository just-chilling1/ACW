...
export function HooksCtaPanel({ assets }: HooksCtaPanelProps) {
    const hooks = assets.filter((a) => a.kind === "hook");
    const ctas = assets.filter((a) => a.kind === "cta");
    ...
    return (
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
            <section>
                ...
                <div className="flex flex-col gap-2">
                    {hooks.slice(0, 12).map((hook) => (
                        <div key={hook.id} className="card-base flex items-start justify-between gap-3 p-3">
                            <div>
                                {hook.meta?.category ? (
            ...
            <section>
                ...
                <div className="flex flex-col gap-2">
                    {ctas.map((cta) => (
                        <div key={cta.id} className="card-base flex items-start justify-between gap-3 p-3">
                            <div>
                                {cta.meta?.type ? (
                            ...
                            <CopyButton text={cta.content} label="Copy CTA" />
                        </div>
                    ))}
                </div>
            </section>
        ...
        <div className="flex flex-col gap-6">
            <section className="card-base p-5 sm:p-6">
                ...
                <div className="space-y-4 text-sm leading-relaxed text-text-secondary">
                    ...
                    <p><strong className="text-text-primary">Where to promote:</strong> {campaign.strategy?.whereToPromote}</p>
                        ...
                        return (
                            <div key={cat.key} className="rounded-[var(--radius-md)] border border-[var(--border-subtle)] p-3">
                                ...
                                <div className="h-1.5 overflow-hidden rounded-full bg-[var(--surface-2)]">
                                    <div className="h-full rounded-full bg-[var(--grad-brand)]" style={{ width: `${val}%` }} />
                                </div>
                            </div>
                        );
                    })}
                ...
                <>
                    ...
                    {activeAsset ? (
                        <article className="card-base p-4 sm:p-5">
                            ...
                            <p className="mb-4 whitespace-pre-wrap text-sm leading-relaxed text-text-secondary">{activeAsset.content}</p>
                            <div className="flex flex-wrap gap-2">
                                <CopyButton text={activeAsset.content} label="Copy & Use" variant="primary" />
                                <button
                                    type="button"
...
export function CampaignLibrary({ campaigns, onDelete, onDuplicate }: CampaignLibraryProps) {
            ...
            {campaigns.map((c) => (
                <article key={c.id} className="card-base flex flex-col gap-3 p-4">
                    ...
                    <div className="mt-auto flex flex-wrap gap-2">
                        ...
                        {onDuplicate ? (
                            <button type="button" onClick={() => onDuplicate(c.id)} className="btn-secondary px-3 py-2 text-xs">
                                Duplicate
                            </button>
                        ) : null}
